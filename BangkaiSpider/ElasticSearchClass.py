...
class ElasticSearchClass(object):
    ...
    def search(self, index, type, body, size=10, scroll='10s'):
        ...
        """
        return self.es.search(index=index, doc_type=type,
                              body=body, size=size, scroll=scroll)
...
# 2.数据的的插入
obj.insertDocument(index="question", type='text', id=9, body={
                   "timestamp": datetime.date(2019,4,4)})

print(obj.searchDoc(index="question", type='text'))
